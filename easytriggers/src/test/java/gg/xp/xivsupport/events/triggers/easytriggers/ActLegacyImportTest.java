...
public class ActLegacyImportTest {
	...
	void testImport() {
		...
		Assert.assertEquals(((CalloutAction) trigger.getActions().get(0)).getText(), "\"qwer\"");
		Assert.assertEquals(((CalloutAction) trigger.getActions().get(0)).getTts(), "\"qwer\"");
	...
	void testImportWithCaptureGroups() {
		...
		Assert.assertEquals(trigger.getEventType(), ACTLogLineEvent.class);
		Assert.assertEquals(((CalloutAction) trigger.getActions().get(0)).getText(), "Hey, {match.group(2)} seconds until {match.group('verb')}!");
		Assert.assertEquals(((CalloutAction) trigger.getActions().get(0)).getTts(), "Hey, {match.group(2)} seconds until {match.group('verb')}!");
