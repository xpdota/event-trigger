...
public class TextFieldAsync<X> extends JTextField implements ResettableField {

	private static final Logger log = LoggerFactory.getLogger(TextFieldAsync.class);

	private static final ExecutorService exs = Executors.newCachedThreadPool(Threading.namedDaemonThreadFactory("TextFieldAsync"));
	...
	public TextFieldAsync(Function<String, X> parser, Consumer<X> consumer, String initialValue) {
		this(parser, consumer, () -> initialValue);
	}

	public TextFieldAsync(Function<String, X> parser, Consumer<X> consumer, Supplier<String> valueGetter) {
		super(10);
		this.parser = parser;
	...
	private void update() {
		...
		exs.submit(() -> {
			...
			String latestText = getText();
			// Bail out if text changed in the mean time, because it's pointless to process something
			// that's already out of date.
			if (!Objects.equals(currentRawText, latestText)) {
				return;
			}
			try {
