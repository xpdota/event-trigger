...
public class EnhancedWindowUtils {
	...
	private static class X11WindowUtils extends NativeWindowUtils {
		...
		private void setWindowShape(final Window w, final PixmapSource src, int whichShape) {
			Runnable action = new Runnable() {
				@Override
				public void run() {
					X11 x11 = X11.INSTANCE;
					...
					try {
						...
						ext.XShapeCombineMask(dpy, win, whichShape,
		...
		@Override
		protected void setMask(final Component w, final Raster raster, int whichShape) {
			setWindowShape(getWindow(w), new PixmapSource() {
				@Override
				public Pixmap getPixmap(Display dpy, X11.Window win) {
					return raster != null ? createBitmap(dpy, win, raster) : null;
	 ...
	 */
	public static void setWindowMask(Window w, Shape mask, int shapeInput) {
		getInstance().setWindowMask(w, mask, shapeInput);
	}
	...
	public static void setComponentMask(Component c, Shape mask) {
		getInstance().setWindowMask(c, mask, Xext.ShapeBounding);
	}
